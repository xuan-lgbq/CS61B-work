...
public class TestGuitarString  {
    ...
    @Test
    public void testPluckTheAStringforLinkedList() {
        GuitarStringforLinkedListDeque aString = new GuitarStringforLinkedListDeque(GuitarHeroLite.CONCERT_A);
        aString.pluckforLinkedList();
        for (int i = 0; i < 50000; i += 1) {
            StdAudio.play(aString.sampleforLinkedList());
            aString.ticforLinkedList();
        }
    }

    @Test
    public void testSampleforLinkedList() {
        GuitarStringforLinkedListDeque s = new GuitarStringforLinkedListDeque(100);
        assertEquals(0.0, s.sampleforLinkedList(), 0.0);
        assertEquals(0.0, s.sampleforLinkedList(), 0.0);
        assertEquals(0.0, s.sampleforLinkedList(), 0.0);
        s.pluckforLinkedList();
        double sample = s.sampleforLinkedList();
        assertNotEquals("After plucking, your samples should not be 0.", 0.0, sample);

        assertEquals("Sample should not change the state of your string.", sample, s.sampleforLinkedList(), 0.0);
        assertEquals("Sample should not change the state of your string.", sample, s.sampleforLinkedList(), 0.0);
    }


    @Test
    public void testTicforLinkedList() {
        GuitarStringforLinkedListDeque s = new GuitarStringforLinkedListDeque(100);
        assertEquals(0.0, s.sampleforLinkedList(), 0.0);
        assertEquals(0.0, s.sampleforLinkedList(), 0.0);
        assertEquals(0.0, s.sampleforLinkedList(), 0.0);
        s.pluckforLinkedList();
        double sample1 = s.sampleforLinkedList();
        assertNotEquals("After plucking, your samples should not be 0.", 0.0, sample1);

        s.ticforLinkedList();
        assertNotEquals("After tic(), your samples should not stay the same.", sample1, s.sampleforLinkedList());
    }


    @Test
    public void testTicCalculationsforLinkedList() {
        // Create a GuitarString of frequency 11025, which
        // is a Deque of length 4.
        GuitarStringforLinkedListDeque s = new GuitarStringforLinkedListDeque(11025);
        s.pluckforLinkedList();

        // Record the front four values, ticcing as we go.
        double s1 = s.sampleforLinkedList();
        s.ticforLinkedList();
        double s2 = s.sampleforLinkedList();
        s.ticforLinkedList();
        double s3 = s.sampleforLinkedList();
        s.ticforLinkedList();
        double s4 = s.sampleforLinkedList();

        // If we tic once more, it should be equal to 0.996*0.5*(s1 + s2)
        s.ticforLinkedList();

        double s5 = s.sampleforLinkedList();
        double expected = 0.996 * 0.5 * (s1 + s2);
